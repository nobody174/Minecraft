...
public final class RimBoostEffect {
    ...
    private static final ResourceLocation SHADER_LOCATION =
        ResourceLocation.fromNamespaceAndPath("foundya", "rim_boost");
    ...
    public static void register(IEventBus modEventBus) {
        ...
        NeoForge.EVENT_BUS.addListener(RimBoostEffect::onLoggingOut);
    }

    /** Frees the offscreen scratch target's GPU resources on disconnect rather than leaking them until the next resize. */
    ...
    private static void onRegisterShaders(RegisterShadersEvent event) {
        try {
            ...
            org.slf4j.LoggerFactory.getLogger("FoundYa").error("Failed to load rim_boost shader", e);
        }
    }
    ...
    private static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        if (scratchTarget == null) {
            scratchTarget = new TextureTarget(width, height, false, false);
        }
