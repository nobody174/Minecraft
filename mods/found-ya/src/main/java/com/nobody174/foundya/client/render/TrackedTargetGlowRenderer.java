...
package com.nobody174.foundya.client.render;
 ...
 * Found Ya!'s actual visual identity (see docs/UI_STYLE_GUIDE.md); this
...
public final class TrackedTargetGlowRenderer {
    ...
    public static void register(IEventBus modEventBus) {
        ...
        NeoForge.EVENT_BUS.addListener(TrackedTargetGlowRenderer::onLoggingOut);
    }

    /**
     * Clears the per-texture {@link RenderType} cache on disconnect —
     * it grows for the lifetime of the client session (one entry per
     * distinct mob texture the player has locked or search-revealed),
     * so a fresh session shouldn't carry over the previous world's set.
     */
    private static void onLoggingOut(ClientPlayerNetworkEvent.LoggingOut event) {
        ADDITIVE_RIM_TYPES.clear();
    }
    ...
    private static void onRenderLivingPost(RenderLivingEvent.Post event) {
        ...
        if (isLocked) {
            ...
            poseStack.popPose();
            if (FoundYaConfig.isRimBoostEnabled()) {
    ...
    private static RenderType additiveRim(ResourceLocation texture) {
        return ADDITIVE_RIM_TYPES.computeIfAbsent(texture, location -> {
            ...
            return RenderType.create("foundya_additive_rim", DefaultVertexFormat.NEW_ENTITY, VertexFormat.Mode.QUADS, 256, true, true, state);
        });
    }
    ...
    private static RenderType silhouette() {
        if (silhouetteType == null) {
            ...
            silhouetteType = RenderType.create("foundya_silhouette",
