...
package com.nobody174.foundya.client.hud;
...
import com.nobody174.foundya.tracking.TargetState;
import com.nobody174.foundya.tracking.TrackedTargetManager;

/**
 * Draws Found Ya!'s actual visual identity: a corner-bracket reticle
...
public final class TrackerHudOverlay {
    ...
    private static final int BEACON_GRADIENT_SEGMENTS = 12;
    ...
    private static void onRenderGuiPost(RenderGuiEvent.Post event) {
        ...
        Entity target = TrackedTargetManager.findLockedEntity(mc.level);
        if (target == null) {
            return;
        }
        ...
        int accentColor = state == TargetState.TRACKING
            ? FoundYaConfig.getTrackingAccentColor()
            : state.colorRgb();

        float nearDistance = FoundYaConfig.getNearDistance();
        float farDistance = FoundYaConfig.getFarDistance();
        ...
        boolean useBeacon = FoundYaConfig.isBeaconEnabled()
            && distance > FoundYaConfig.getBeaconDistance()
    ...
    private static void drawReticle(GuiGraphics graphics, float centerX, float centerY, int accentColor, float alpha, float scale) {
        ...
        float half = (FoundYaConfig.getBracketBaseSize() * scale) / 2.0F;
     ...
     * {@code FoundYaConfig.getBeaconDistance()}): a tall vertical accent
    ...
    private static void drawBeacon(GuiGraphics graphics, float baseX, float baseY, int accentColor, float alpha) {
        ...
        int left = (int) baseX - half;
        int right = left + BEACON_STROKE;
        float segmentHeight = (float) BEACON_HEIGHT / BEACON_GRADIENT_SEGMENTS;

        for (int i = 0; i < BEACON_GRADIENT_SEGMENTS; i++) {
            float fadeStart = 1.0F - i / (float) BEACON_GRADIENT_SEGMENTS;
            float fadeEnd = 1.0F - (i + 1) / (float) BEACON_GRADIENT_SEGMENTS;
            int colorStart = withAlpha(accentColor, alpha * fadeStart);
            int colorEnd = withAlpha(accentColor, alpha * fadeEnd);

            int yStart = (int) baseY - Math.round(i * segmentHeight);
            int yEnd = (int) baseY - Math.round((i + 1) * segmentHeight);
            graphics.fillGradient(left, yEnd, right, yStart, colorEnd, colorStart);
        }
    ...
    private static void drawDistanceText(GuiGraphics graphics, float centerX, float centerY, float distance, float alpha) {
        ...
        int y = Math.round(centerY) + FoundYaConfig.getBracketBaseSize() / 2 + 4;
    ...
    private static void drawOffScreenCaret(GuiGraphics graphics, ScreenProjection.Result projection, int accentColor, float alpha) {
        ...
        if (dx == 0 && dy == 0) {
            dx = DIRECTION_EPSILON;
        }
        float bearing = (float) Math.atan2(dy, dx);

        float halfW = centerX - EDGE_MARGIN;
        float halfH = centerY - EDGE_MARGIN;
        float scaleX = halfW / Math.abs(dx == 0 ? DIRECTION_EPSILON : dx);
        float scaleY = halfH / Math.abs(dy == 0 ? DIRECTION_EPSILON : dy);
