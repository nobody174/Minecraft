...
### Fixed — Abandoned pets stayed frozen instead of becoming a free wild mob
- A real test showed that abandoning ownership (the 3rd left-click in the Stay/Follow cycle) still left the pet standing motionless even after attacking it repeatedly. Root cause: `abandonOwnership` called `PetBehaviorController.applyMode(target, PetBehaviorMode.STAY)` after clearing ownership, which calls `setNoAi(true)` — permanently freezing a pet that's supposed to be a normal, free-roaming wild mob again. Now restores full vanilla AI (`setNoAi(false)`) directly and removes the behavior-mode/toggle-count attachments entirely instead.

### Changed — Evolution cap expanded from 3 levels to 10
- A real test pet accumulated 15,000+ XP while stuck at the old cap (stage 2, level 3 — XP thresholds only went up to 300) with nowhere for the XP to go. `EvolutionRules.MAX_STAGE` raised from 2 to 9 (10 total levels), with a roughly-doubling XP curve (100/180/324/583/1049/1888/3398/6116/11008) replacing the old 100/300 two-threshold table. Skill-slot unlock levels adjusted from {1,1,2,3,3} to {1,1,4,8,8} so all 5 skills are available before the final level rather than only at it.
- **Fixed alongside this**: `PetData.withXp` only checked the single next XP threshold per call (an `if`, not a loop), so a pet sitting on a large XP surplus could only climb one stage at a time no matter how much excess XP it had — found because the same 15,000+ XP test pet would have needed many small XP grants just to catch up to its new cap. Now loops through every eligible stage-up in one pass.

### Added — defensive debounce on pet release
- A report of "releasing 2 pets at once" turned out, on investigation, to be 2 deliberate separate right-clicks (one per filled vessel in two different hotbar slots) — correct expected behavior, not a bug. Added a same-tick debounce to `CaptureBallItem.useOn` anyway as a harmless safety net, since vanilla block interaction dispatch is only supposed to fire once per click and this guards against any genuine future double-dispatch without changing normal single-click-per-vessel behavior.

### Confirmed (no fix needed) — per-level stat scaling and breeding
- An initial report of identical tooltip stats between a level-0 and level-10 pet turned out to be comparing pet/ball state from before the evolution-cap fix was deployed. A clean retest (fresh pet, leveled to max via `/petevolution xp`) confirmed stats scale correctly at every step across the new 10-level curve.
- Breeding confirmed fully working end-to-end for the first time: two RARE cows leveled to max and placed nearby produced a 3rd RARE cow at level 0 shortly after. Phase 2 (battle, rarity, breeding) is now completely manually verified.
