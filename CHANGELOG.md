...
- **Known limitation** (documented in `render/CaptureBallRenderNotes.java` and `FUTURE_FEATURES.md`): the 4 rarity-variant textures are generated and committed but NOT yet wired up to switch dynamically based on `PetData.rarity()`. Doing so in NeoForge 1.21.1 requires either a custom `ItemModel` implementation reading the component at render time, or mapping rarity to `custom_model_data` for a `range_dispatch` — both are real render-pipeline changes with meaningful risk of subtly breaking item rendering, and unverifiable without launching the game in this autonomous, no-game-launch run. Shipped the single base texture now per the task's own fallback guidance.
